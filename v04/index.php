...
#-----------------------------------------
#
# ViteX FullNode Explorer v3
# Developed by Chris & Allen @ViteLabs
#
#-----------------------------------------
...
#VARS:
$addressVerified=0;
$vite_block_explorer_addr="https://explorer.vite.net/account/"; #eg. vite_f3593e633218ba13b2a6a6379d87e588285a5fba778dd3c20a
...
function isValidHex($hexAddr) {
	...
	$ADDR_LEN = strlen($ADDR_PRE) + $ADDR_SIZE * 2 + $ADDR_CHECK_SUM_SIZE * 2;
    return strlen($hexAddr) == $ADDR_LEN && strpos($hexAddr, $ADDR_PRE) == 0;
}
...
function verifyAddress($literalAddress) { 
 $ADDR_PRE = 'vite_';
 $ADDR_SIZE = 20;
 $ADDR_CHECK_SUM_SIZE = 5;
 $ADDR_LEN = strlen($ADDR_PRE) + $ADDR_SIZE * 2 + $ADDR_CHECK_SUM_SIZE * 2;

	if(!strlen($literalAddress)===$ADDR_LEN){ return false; } else {
		$addrBody=substr($literalAddress,strlen($ADDR_PRE),40);
		$checkSum=substr($literalAddress,-10);
		// echo date('h:i:s:u', time()) . ": Literal Address : ". $literalAddress.". <br>"; 
		// echo date('h:i:s:u', time()) . ": Address Body    : ". $addrBody .". <br>"; 
		// echo date('h:i:s:u', time()) . ": Address Checksum: ". $checkSum .". <br>"; 
		
		$blake2b = new Blake2b($ADDR_CHECK_SUM_SIZE); 
		
		#This function was corrected by Allen@ViteLabs, 17.11.2020:
		#Allen's:
		#$_checkSum = $blake2b->hash(hex2bin($addrBody)); 
		#Mine:
		#$_checkSum = $blake2b->hash($addrBody);
		
		try {
			$hex=@hex2bin($addrBody);
			if(false === $hex) {
				throw new Exception("Invalid hexedecimal value.");
			}
			$_checkSum = $blake2b->hash($hex); 
		}
		catch (exception $e) {
			return false;
		}
		
		if ($checkSum == bin2hex($_checkSum) || $checkSum == bin2hex(~$_checkSum)) 
		 return true; 
		else  
		 return false; 
	}
...
function cleanupCycleDir($days) {
	// echo date('h:i:s:u', time()) . ": Removing old cycle files - START <br>"; 
	$folderName='cycles/';
	if (file_exists($folderName)) {
		foreach (new DirectoryIterator($folderName) as $fileInfo) {
			...
			if ($fileInfo->isFile() && time() - $fileInfo->getCTime() >= $days){# *24*60*60) { #rm cycle files not used for $days 
				unlink($fileInfo->getRealPath());
			}
		}
	}
	// echo date('h:i:s:u', time()) . ": Removing old cycle files - END <br>"; 
}
...
function getJsonCycle($cycle_number){
	...
	$cycle_incomplete=floor((time() - 1558411200) / 24 / 3600);
	$cycle_complete=floor((time() - 1558411200) / 24 / 3600)-1;
	/*
	echo date('h:i:s:u', time()) . ": Cycle incomplete: <br>";
	echo "<a href='$fullRealUrl$cycle_incomplete' target='_blank'>$fullRealUrl$cycle_incomplete</a><br>";
	echo "<br>";
	echo date('h:i:s:u', time()) . ": Cycle complete: <br>";
	echo "<a href='$fullRealUrl$cycle_complete' target='_blank'>$fullRealUrl$cycle_complete</a><br>";
	echo "<br>";
	echo "<br>";
	*/
	// if user wants to check current incomplete cycle, 
	// we want to remove old json copy, and download a fresh json copy every time:
	if ($cycle_number == $cycle_incomplete && file_exists('cycles/'.$file_name))
	{
		// echo date('h:i:s:u', time()) . ": Removing copy of current incomplete json file: ".$file_name." <br>";
		if (!unlink('cycles/'.$file_name)) {  
			echo ("+ERR: Cycle file cannot be deleted due to an error. <br>Please raise an issue on GitHub.<br><a href='https://github.com/GreedyBastard/vite'>https://github.com/GreedyBastard/vite</a><br><br>");  
		}  
	}
	
	// check if such (old) cycle json file was already downloaded before, +90% faster:
	if (file_exists('cycles/'.$file_name)) {   
		echo date('h:i:s:u', time()) . ": File ".$file_name." already exists. <br>"; 
		$json = file_get_contents('cycles/'.$file_name);
		$json_output = (json_decode($json));
		return $json_output;
	} else {
		// in case (old) cycle json file does not exist... download it:
		// echo date('h:i:s:u', time()) . ": File ".$file_name." not found. Downloading..<br>"; 
		...
		<div class="col-md-5">
			<form role="form" action="index.php" method="POST">
				<div class="form-group">
					...
					<input type="text" class="form-control" id="vite_addr" name="vite_addr" placeholder="Please enter FullNode Vite address" <?php if(isset($_POST['vite_addr'])){echo 'value="'.$_POST['vite_addr'].'"';}?> >
				</div>
		</div>
		<div class="col-md-1">
				<div class="form-group">
	...
	if(isset($_POST['submit'])){
		...
		if(isset($_POST['vite_addr'])){
			if (strpos($_POST['vite_addr'], "vite_") === 0) {
				...
				if(verifyAddress($vite_addr)){
					$msg01=$inf_div . "Provided FullNode Vite address:     ".$vite_addr."</br></br></div>";
					$addressVerified=1;
				} else {
					$addressVerified=0;
					$msg01=$err_div . "You haven't provided <b>correct</b> FullNode vite address.</br>"."</div>";
				}
			} else {
				$addressVerified=0;
				$msg01=$err_div . "You haven't provided <b>correct</b> FullNode vite address.</br>"."</div>";
		...
		if(isset($_POST['cycle'])){
			if( is_numeric($_POST['cycle'])){
				...
				else if ($_POST['cycle'] == $cycle_incomplete) {
					$cycle=$cycle_incomplete;
					$msg02=$inf_div . "You haven't provided any cycle number.<br/> Using current incomplete cycle instead: ".$cycle."</div>";
				}
				else {
				$cycle=$cycle_incomplete;
...
if($addressVerified==1)
{
	...
	if(true==($json2 = file_get_contents("https://stats.vite.net/api/getAlivePeers?address=".$vite_addr))){
			...
			//failsafe in case API returns 0 nodes in "data" key:
			if(count($json_output->data) == 0){
				echo date('h:i:s:u', time()) . ": Full/Real API returns 0 nodes in 'data' key. <br><br><br>"; 
			}
