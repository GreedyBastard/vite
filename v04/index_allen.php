...
function getJsonCycle($cycle_number, $incomplete_number){
	
	$last_complete_cycle = $incomplete_number - 1;
	$file_name = "cycle".$cycle_number.".json"; 
	$last_complete_file_name = "cycle".$last_complete_cycle.".json"; 

	// check if such cycle was already downloaded before, +90% faster
	// any request to an incomplete cycle needs to pull the latest data from the API
	if ($incomplete_number == $cycle_number) {
		
		if (!file_exists('cycles/'.$last_complete_file_name)) {
			// last cycle - get data from web and save
			getJsonFromUrl($last_complete_cycle, true);
		}
		// get the incomplete cycle from web, no need to save file
		return getJsonFromUrl($cycle_number, false);
	}
	if (!file_exists('cycles/'.$file_name)) {   
		// echo date('h:i:s:u', time()) . ": File not found. Downloading..<br>"; 
		// save to file
		return getJsonFromUrl($cycle_number, true);
		#}
	} else {
...
function getJsonFromUrl($cycle_number, $saveFile) {
	$fullRealUrl = 'https://rewardapi.vite.net/reward/full/real?cycle='; 
	$file_name = "cycle".$cycle_number.".json"; 
	// Use file_get_contents() function to get the file 
	// from url and use file_put_contents() function to 
	// save the file on webserver
	if($saveFile && file_put_contents( 'cycles/'.$file_name,file_get_contents($fullRealUrl.$cycle_number))) { 
...
{
	$json_output=getJsonCycle($cycle, $cycle_incomplete);
